...
oc get secret -l "${LABEL_SELECTOR}" -o yaml -n "${SOURCE_NAMESPACE}" | yq eval "del(.items[].metadata.namespace,.items[].metadata.ownerReferences,.items[].metadata.managedFields,.items[].metadata.resourceVersion,.items[].metadata.uid)" - > $OUTPUT_NAME
