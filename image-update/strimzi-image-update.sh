#!/usr/bin/env bash
# Skopeo needs to be installed as prerequisite
...
for YAML_BUNDLE_PATH in "${YAML_BUNDLE_PATH_ARRAY[@]}"; do
    ...
    for C_FILE in "${FILES[@]}"; do
        ...
        if [[ $C_FILE == *"Crd"* ]] || [[ $C_FILE == *"ClusterRole-"* ]]; then
            ...
            if test -f "$TARGET_DIR/$SHARED_YAML_BUNDLE_FILES/$C_FILE"; then
                ...
                echo "File $TARGET_DIR/$SHARED_YAML_BUNDLE_FILES/$C_FILE does not exist. Skipping metadata backup."
            fi
            ...
            if test -f "$TARGET_DIR/$YAML_BUNDLE_PATH/$C_FILE"; then
                ...
                echo "File $TARGET_DIR/$YAML_BUNDLE_PATH/$C_FILE does not exist. Skipping metadata backup."
            fi
    ...
    # Restore / keep certain environment vars & spec details from the original
...
IMAGES_TAGS=$(
    cat "${YAML_BUNDLE_PATH_ARRAY[0]}/${DEPLOYMENT_FILE_NAME}" |
        grep "$TARGET_ORG_REPO"/ |
        sort -u |
        awk '{$1=$1};1' |
        cut -d ' ' -f2- |
        cut -d '/' -f3- |
...
for ELEMENT_O in "${IMGS[@]}"; do
    ...
    for YAML_BUNDLE_PATH in "${YAML_BUNDLE_PATH_ARRAY[@]}"; do
        if [[ "$IMAGE_NAME" == *"bridge"* ]]; then
            ...
            echo "[INFO] Skipping STRIMZI_DEFAULT_KAFKA_BRIDGE_IMAGE or any 'bridge' reference."
            continue
        else
            echo "[INFO] Changing ${IMAGE_NAME} tag to ${NEW_DIGEST}"
...
for ELEMENT in "${IMAGES[@]}"; do
    ...
    if [[ "$ELEMENT" == *"="* ]]; then
        ...
        LATEST_DIGEST=$(skopeo inspect \
            --override-arch amd64 \
            --override-os linux \
            docker://"$TARGET_ORG_REPO"/"$IMAGE":latest-kafka-"$PREFIX" \
            --format "{{ .Digest }}")
    elif [[ "$ELEMENT" == *"kafka@"* ]]; then
        # We do not replace lines that directly have "kafka@" in them
        continue
